...
export function yOf(z: number): number {
  return HORIZON + (KID_Y - HORIZON) * (Z0 / (Z0 + Math.max(0, z)));
}
/** z[px先] → 大きさの比（1が足元）。 */
export function scaleOf(z: number): number {
  return Z0 / (Z0 + Math.max(0, z));
}
 ...
 * **2レーン。左か右か、それだけ。**
 *
 * 3レーンにすると「真ん中に居たい」が生まれ、そのために
 * 指を離す＝顔が上がる、という要らない駆け引きが混ざる。
 * 2つなら**触っている側がそのまま自分の居場所**で、迷いが無い。
 *
 * そして、触る場所は画面の左半分か右半分。**幅195pxの的が2つ**しかない。
 * 旧版で捨てたかったのは座標そのものではなく、
 * 「あと3px 左にいれば助かった」が生まれる細かさのほう。
 */
export const LANES = 2;
/** 足元でのレーンの中心x。 */
export const LANE_X = [56, 104] as const;
