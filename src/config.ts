 ...
 * 攻めと守りの差が消える。
 *
 * 14では足りなかった。塊の中心を帯の縁ぎりぎりから引くようにした（EDGE_MARGIN）ので、
 * 粒が縁ちょうどまで来る。半幅14だと、線から10pxずれた時点で当たる計算になり、
 * 回廊をなぞるボットの被弾が走行ごとに0〜3回で揺れた。
 * 「なぞれば無傷」は保証なので、揺れる時点で足りていない。16にした。
 */
export const CORRIDOR_HALF_MIN = 16;
